...
def covid_api_data_hospital_cases(data:list) -> str: 
    ...
    that_days_data = data[y]  
    #goes through each days hospital cases data until that data does not equal none
    while that_days_data['hospitalCases'] == None: 
        y += 1
        that_days_data = data[y] 
...
def covid_api_data_new_cases_that_week_nation(data:list) -> int: 
    ...
    Description:
        ...
        Function which gets the 7 day infection rate for the specified nation from the covid api data
    ...
    while that_days_data['newCasesByPublishDate'] == None: 
        y += 1
        that_days_data = data[y]
    x = 0 
...
def covid_api_data_new_cases_that_week_else(data:list) -> int:
    ...
    Description:
        ...
        Function which gets the 7 day infection rate for the specified local area from the covid api data
    ...
    that_days_data = data[y]
    #goes through each days cases until it does not equal none
    while that_days_data['newCasesBySpecimenDate'] == None:
        y += 1
        that_days_data = data[y]
...
def call_function() -> list:  
    ...
    Returns:
        ...
        data {list} : a list of covid api data: local 7 day infection rate, national 7 day infection rate,
                                                national hospital cases, national deaths
    
    '''
    data = [covid_api_data_new_cases_that_week_nation(covid_api_request(config_data["local location"],)),
    covid_api_data_new_cases_that_week_else(covid_api_request(config_data["nation location"],'nation')), 
    covid_api_data_hospital_cases(covid_api_request(config_data["nation location"],'nation')), 
    covid_api_data_deaths(covid_api_request(config_data["nation location"],'nation'))] 
    return data
